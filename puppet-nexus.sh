...
curl -sS ${REDIRECT_URL} ${OUT} ${AUTHENTICATION} -v -R --location-trusted --fail  2> /dev/null > /dev/null
if [ $? -eq 0 ] ; then 
	echo " " 
	echo "###########################################" 
	echo "Download completed successfully from Nexus." 
	echo "###########################################" 
	echo " " 
else
	echo " " 
	echo "#############################" 
	echo "Failed to download from Nexus" 	
	echo "#############################" 
	echo " " 
	exit 1
fi 
# Stop all agent 
#echo "Stopping agent to filter."
#su - peadmin -c "mco rpc service stop service=pe-puppet"
# Force Puppet Agent 
#	echo "Executing Deploy on JBoss Servers - ${TAG} ..." 
#su - peadmin -c "mco puppet runonce --with-class 'jboss' -v --tag deploy_${TAG} --environment ${TAG} --with-identity ${NODES}"
	echo "Executing Deploy on ${NODES} ... "
for SRVJBOSS in ${NODES} ; do 
	echo " " 
	echo "Init Deploy on ${SRVJBOSS} ... "
	ssh puppet-deploy@${SRVJBOSS} "sudo puppet agent -t " 2> /dev/null > /dev/null
        STATUS=$?
	if [ "$STATUS" -eq "0" ] || [ "$STATUS" -eq "2" ]; then 
                echo " " 
                echo "#########################################################" 
                echo "Deploy completed successfully on \"${SRVJBOSS}\"."
                echo "#########################################################" 
                echo " " 
		...
		echo " " 
		echo "###################################" 
		echo "Failed to deploy on \"${SRVJBOSS}\"" 	
		echo "###################################" 
		echo " " 
...
# Verifica se ocorreu falha em algum deploy.
test ! -z ${DEPLOYPROBLEM} && exit 1 || exit 0 
